...
const KeyPrefix = "/kim/prpc/"

// Register ...
type Register struct {
	Options
	cli                 *clientv3.Client
	serviceRegisterCh   chan *registry.Service
	serviceUnRegisterCh chan *registry.Service
	lock                sync.Mutex
...
// NewETCDRegister ...
func NewETCDRegister(opts ...Option) (registry.Registrar, error) {
	opt := defaultOption
	for _, o := range opts {
		o(&opt)
	}
	...
	r := &Register{
		Options:             opt,
		serviceRegisterCh:   make(chan *registry.Service),
		serviceUnRegisterCh: make(chan *registry.Service),
		lock:                sync.Mutex{},
		downServices:        atomic.Value{},
	...
	for {
		...
		case service := <-r.serviceUnRegisterCh:
			if _, ok := r.registerServices[service.Name]; !ok {
				log.Error(
					context.TODO(),
					"UnRegisterService err", log.String("service_name", service.Name),
				)
				continue
			}
...
func (r *Register) unRegisterService(ctx context.Context, service *registry.Service) {
...
func (r *Register) Register(ctx context.Context, service *registry.Service) {
	r.serviceRegisterCh <- service
}

func (r *Register) UnRegister(ctx context.Context, service *registry.Service) {
	r.serviceUnRegisterCh <- service
}

func (r *Register) GetService(ctx context.Context, name string) *registry.Service {
...
func (r *Register) updateDownService(service *registry.Service) {
	r.lock.Lock()
	defer r.lock.Unlock()

	downServices := r.downServices.Load().(map[string]*registry.Service)
...
func (r *Register) delDownService(service *registry.Service) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *Register) getDownServices() map[string]*registry.Service {
	allServices := r.downServices.Load()
	if allServices == nil {
		return make(map[string]*registry.Service, 0)
	}

	return allServices.(map[string]*registry.Service)
}
